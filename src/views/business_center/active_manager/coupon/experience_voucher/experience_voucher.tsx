...
import { experienceVoucherGet, queryDictList, experienceVoucherOperate } from '../../../../../server/api'
...
class CrystalCoupon extends React.PureComponent<IProps, IState> {
  ...
  private async getList() {
    ...
    let response = await experienceVoucherGet(params);
  ...
  private async operateCard(info: any) {
    ...
    let res = await experienceVoucherOperate({ id: info.id })
    if (res.code !== 200) {
      message.error(res.message)
      return
    }
  ...
  render() {
    const { list, isViewOnly, editInfo, total,
      currentPage, pageSize, visible, isShowGrandDetail, grandDetailId, sellChannel, sellChannelList } = this.state
    const columns:any = [
      {
        title: '序号',
        dataIndex: 'id',
    ...
    return (
      <div className='page' style={{ paddingTop: 0 }}>
        ...
        />
        {visible && <CardEdit type={4} isViewOnly={isViewOnly} editInfo={editInfo} onClose={this.closeEditDialog.bind(this)} submitSuccess={this.submitSuccess.bind(this)} />}
        {
          isShowGrandDetail && <GrandDetail type={4} id={grandDetailId} onClose={this.closeGrandDetail.bind(this)} />
        }
      </div>
    )
  }
