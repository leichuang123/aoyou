...
import { getTimeQuantumOrder, operateOrderState, getProductTypeList, queryDictList, resetOrderOldDevice, unBundling, queryDataCenterList,} from '../../../../server/api'
...
class RealTimeOrder extends React.PureComponent<IProps, IState> {
  ...
  private async unbindDevice (info: any) {
    let sure = await new Promise(resolve => {
      Modal.confirm({
        title: '温馨提示',
        content: `确认解绑？`,
  ...
  render() {
    const { list, currentPage, total, pageSize, visible, editInfo, isShowRenewDialog, reNewProductNo, reNewOrderNo,
      isShowChangeDevice, changeDeviceInfo, sellChannelList,
      //  orderChannel,  
      } = this.state
    // const columns: any = [
    const columns: any = [
      ...
      },
      {
        ...
        render: (text: any, item: any) => <div className='flex-enter'>
          {
            item.durationOrderState === 1 && <Button type='link' onClick={this.operState.bind(this, item)}>停用</Button>
          }
          {
            item.durationOrderState === 2 && <Button type='link' onClick={this.operState.bind(this, item)}>启用</Button>
          }
          <Button type='link' onClick={this.orderDetail.bind(this, item)}>详情</Button>
          {
            item.durationOrderState !== 0 && <Button type='link' onClick={this.showRenewDialog.bind(this, item)}>续费</Button>
          }
          ...
          {
            (item.durationOrderState !== 0 && item.deviceNo !== '') && <Button type='link' onClick={this.unbindDevice.bind(this, item)}>解绑</Button>
          }
